...
class Time:
    ...
    def debt_rewind(self):
        debt_percent = (BankTasks().onDebtDayEnd() * self.debtrate * (self.debtdays - self.debtdays_recalc))/(365*100)
    ...
    def cridet_rewind(self):
        creditsum = CreditAccount().amount - Account().calcCreditMonthlyPercent * (self.creditmonths - self.creditmonths_recalc)
        return creditsum
